...
from medium_clone_api.settings.local import DEFAULT_FROM_EMAIL
    ...
    def post(self, request, user_id, format=None):
        try:
            ...
            message = (f"Hi there, {profile.user.first_name}!!, the user "
                       f"{user_profile.user.first_name} {user_profile.user.last_name} follows you")
        ...
        if not user_profile.check_following(profile):
            formatted_response = {
                ...
                'message': f"You can't unfollow {profile.user.first_name} {profile.user.last_name}, "
                           f"since you were not following then in the first place"
            }
            return Response(formatted_response, status=status.HTTP_400_BAD_REQUEST)
