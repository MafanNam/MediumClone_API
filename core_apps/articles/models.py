...
class Article(TimeStampedModel):
    ...
    def estimated_reading_time(self):
        return ArticleReadTimeEngine.estimate_reding_time(self)
...
class ArticleView(TimeStampedModel):
    ...
    def __str__(self):
        return (f"{self.article.title} viewed by "
                f"{self.user.first_name if self.user else 'Anonymous'} from IP {self.viewer_ip}")
